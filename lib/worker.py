...
def _generate_error_summary(log_path):
    """Creates a more intelligent error summary."""
    error_lines = []
    try:
    ...
    try:
        ...
        with open(temp_log_path, 'a', encoding='utf-8') as log_file:
            ...
            if os.path.exists(temp_dir_path):
                ...
                mode = job.get('mode')
                if mode == 'music':
                    target_ext = job.get('format', 'mp3')
                elif mode == 'video':
                    target_ext = job.get('format', 'mp4')
                elif mode == 'clip':
                    target_ext = 'mp3' if job.get('format') == 'audio' else 'mp4'
...
def _prepare_job_environment(job, config, log_dir):
    ...
    if job.get("archive"):
        # --- CHANGE: Use the resolved folder name if available ---
        # This ensures that when continuing a download, we find the correct archive file.
...
def yt_dlp_worker(state_manager, config, log_dir, cookie_file_path, yt_dlp_path, ffmpeg_path, stop_event):
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            history_item = {
                "url": job["url"], "title": final_folder or job.get("url"), "folder": final_folder,
